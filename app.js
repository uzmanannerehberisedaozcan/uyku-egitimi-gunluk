...
function formYukle(g) {
  ...
  // Arandı
  const acb=el('arandi-cb'); if(acb) acb.checked=!!g.arandi;
  calcUyanikSureler();
}
...
function gunlukTopla() {
  ...
  // Arandı
  const arandi=el('arandi-cb')?.checked||false;
  return {
...
function uzmanShowDetail(id) {
  ...
  const mevcutListHtml=(n.mevcutDurum||[]).length>0
    ? (n.mevcutDurum||[]).map(s=>`<span style="display:inline-block;background:rgba(196,147,106,0.12);border:1px solid #E2D5C8;border-radius:20px;padding:4px 12px;font-size:12px;margin:3px;">${s}</span>`).join('')
    : '<span style="color:var(--text-soft);font-size:13px;">Henüz seçilmedi</span>';
  const digerListHtml=n.diger?`<div style="margin-top:6px;font-size:13px;color:var(--deep-brown);">📌 ${n.diger}</div>`:'';
  const yaplacaklarHtml=n.yaplacaklar?`<div style="white-space:pre-wrap;font-size:13px;line-height:1.7;color:var(--deep-brown);">${n.yaplacaklar}</div>`:'<span style="color:var(--text-soft);font-size:13px;">Henüz not eklenmedi</span>';

  let html=`<div style="display:flex;gap:14px;align-items:start;flex-wrap:wrap;">
  ...
  // ── SAĞ: NOTLARIM paneli ──
  html+=`<div style="flex:1;min-width:240px;position:sticky;top:20px;align-self:start;">
    <div class="form-card" style="background:linear-gradient(135deg,#FAF6F0,#FDF9F5);">
      <h3 style="display:flex;justify-content:space-between;align-items:center;">
        📋 Notlarım
        <button onclick="uzmanNotDuzenle('${a.id}')" style="font-size:11px;background:none;border:1.5px solid var(--border);padding:4px 10px;border-radius:8px;cursor:pointer;color:var(--mid-brown);">✏️ Düzenle</button>
      </h3>
      <div style="margin-bottom:14px;">
        <div style="font-size:11px;text-transform:uppercase;color:var(--text-soft);font-weight:600;margin-bottom:8px;letter-spacing:.5px;">Mevcut Durum</div>
        <div id="mevcut-list-${a.id}">${mevcutListHtml}${digerListHtml}</div>
      </div>
      <div>
        <div style="font-size:11px;text-transform:uppercase;color:var(--text-soft);font-weight:600;margin-bottom:8px;letter-spacing:.5px;">Yapılacaklar</div>
        <div id="yaplacaklar-list-${a.id}">${yaplacaklarHtml}</div>
      </div>
    </div>
  </div>
  </div>`; // flex wrapper kapandı
...
async function uzmanNotKaydet(id) {
  ...
  try {
    ...
    // Görünümü güncelle
    const TÜMSEÇENEKLER=[...SECENEKLER];
    const mevcutListHtml=secili.length>0
      ? secili.map(s=>`<span style="display:inline-block;background:rgba(196,147,106,0.12);border:1px solid #E2D5C8;border-radius:20px;padding:4px 12px;font-size:12px;margin:3px;">${s}</span>`).join('')
      : '<span style="color:var(--text-soft);font-size:13px;">Henüz seçilmedi</span>';
    const digerHtml=diger?`<div style="margin-top:6px;font-size:13px;color:var(--deep-brown);">📌 ${diger}</div>`:'';
    const ml=el('mevcut-list-'+id); if(ml) ml.innerHTML=mevcutListHtml+digerHtml;
    const yl=el('yaplacaklar-list-'+id);
    if(yl) yl.innerHTML=yaplacaklar?`<div style="white-space:pre-wrap;font-size:13px;line-height:1.7;color:var(--deep-brown);">${yaplacaklar}</div>`:'<span style="color:var(--text-soft);font-size:13px;">Henüz not eklenmedi</span>';
