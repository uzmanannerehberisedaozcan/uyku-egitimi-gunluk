...
function formYukle(g) {
  ...
  if(rl&&g.ruyaOgunleri&&g.ruyaOgunleri.length>0){
    rl.innerHTML=''; ruyaCounter=0;
    g.ruyaOgunleri.forEach((r,i)=>{ addRuya(); const rid='ry-'+i; if(el(rid+'-saat')) el(rid+'-saat').value=r.saat||''; });
  }
  calcUyanikSureler();
}
...
async function uzmanLoadAileler() {
  ...
  try {
    ...
    for (const ds of snap.docs) {
      ...
      // Uzman notlarını çek
      let uzmanNot = {mevcutDurum:[], yaplacaklar:'', diger:''};
      try {
        const ns=await getDoc(doc(db,'uzman_notlari',ds.id));
        if(ns.exists()) uzmanNot=ns.data();
      } catch(e){}
      uzmanAileler.push({
